...
class ContagionModel(NetworkModel):
    ...
    def avg_income_of_spread(self, dataset_size=1000, cap=0.9, mode='max'):
        # avg time to spread over the dataset.
        # The time to spread is measured in one of the modes:
        # integral, max, and total.
        ...
        if mode == 'integral':
            ...
            sum_of_integrals = 0
            for i in range(dataset_size):
                _, _, _, infected_fraction_timeseries = self.discount_the_total_spread(cap=cap, get_utility_series=True)
                integral = sum(infected_fraction_timeseries)
                sum_of_integrals += integral
                integrals += [integral]

            avg_income = sum_of_integrals/dataset_size
            income_std = np.std(integrals)
            income_max = np.max(integrals)
            income_min = np.min(integrals)
            income_samples = np.asarray(integrals)

        elif mode == 'max':

                cap_utilities = []
                sum_of_cap_utilities = 0
                infection_sizes = []
                sum_of_infection_sizes = 0
                global fraction_evolution
                fraction_evolution = []
                ...
                for i in range(dataset_size):
                    print('dataset_counter_index is:', i)
                    utility, infection_size,l1,l2 = self.discount_the_total_spread(cap=cap, get_utility_series=True)
                    cap_utility = utility
                    if cap_utility == float('Inf'):
                        #dataset_size += -1
                        cap_utilities += [float('Inf')]
                        fraction_evolution += [l2]
                        continue
                    sum_of_cap_utilities += cap_utility
                    cap_utilities += [cap_utility]
                    sum_of_infection_sizes += infection_size
                    infection_sizes += [infection_size]
                    fraction_evolution += [l2]
                ...
                if dataset_size == 0:
                    avg_income = float('Inf')
                    income_std = float('NaN')
                    income_max = float('Inf')
                    income_min = float('Inf')
                    income_samples = np.asarray([float('Inf')])
                    ...
                    avg_income = sum_of_cap_utilities/dataset_size
                    income_std = np.ma.std(cap_utilities)  # masked entries are ignored
                    income_max = np.max(cap_utilities)
                    income_min = np.min(cap_utilities)
                    income_samples = np.asarray(cap_utilities)
            ...
            total_spread_utilities = []
            sum_of_total_spread_utilities = 0
            infection_sizes = []
            sum_of_infection_sizes = 0
            count = 1
            while count <= dataset_size:
                total_spread_utility, infection_size = self.discount_the_total_spread(cap=0.99999, get_utility_series=False)
                if total_spread_utility == float('Inf'):
                    dataset_size += -1
                    total_spread_utilities += [float('Inf')]
                    infection_size += [float('Inf')]
                    print('The contagion hit the time limit.')
                    continue
                total_spread_utilities += [total_spread_utility]
                sum_of_total_spread_utilities += total_spread_utility
                sum_of_infection_sizes += infection_size
                infection_sizes += [infection_size]
                count += 1
            ...
            if dataset_size == 0:
                ...
                avg_income = sum_of_total_spread_utilities/dataset_size
                income_std = np.std(total_spread_utilities)
                income_max = np.max(total_spread_utilities)
                income_min = np.min(total_spread_utilities)
                income_samples = np.asarray(total_spread_utilities)
        ...
        return avg_income, income_std, income_max, income_min, income_samples, \
            avg_infection_size, infection_size_std, infection_size_max, infection_size_min, infection_size_samples, fraction_evolution

    def discount_the_total_spread(self, cap=0.99,
                              get_utility_series=False,
                              verbose=False):
        utility = 0
        time_step = 1
        network_utility_series = []
        fractions_utility_series = []
        DELTA = 0.8
        temp = 0
        self.missing_params_not_set = True
        self.setRandomParams()
        ...
        print('size of self network inside utility_the_total_spread:', sys.getsizeof(self))
        ...
        if get_utility_series:
            network_utility_series.append(dummy_network)
            fractions_utility_series.append(fraction_of_infected)
        ...
        while (total_number_of_infected < cap*self.params['size']) and (not self.spread_stopped):
            self.outer_step()
            dummy_network = self.params['network'].copy()
            utility = utility + (len(self.list_of_active_infected_agents)-temp)*(DELTA)**(time_step-1)
            temp = len(self.list_of_active_infected_agents)
            all_nodes_states = list(
                map(lambda node_pointer: 1.0 * self.params['network'].node[node_pointer]['state'],
                    self.params['network'].nodes()))
            time_step +=1
            ...
            if verbose:
                ...
                print('total_number_of_infected is', total_number_of_infected)
                print('total size is', self.params['size'])
            if time_step > self.params['size']*10:
                utility = float('Inf')
                print('It is taking too hard (10x size) to spread totally.')
                break
        del dummy_network
        if get_utility_series:
            return utility, total_number_of_infected, network_utility_series, fractions_utility_series
        else:
            print(network_utility_series)
            print(fractions_utility_series)
            return utility, total_number_of_infected
