...
###############################################
#check for the cause of different utility when delta=1
# data <- banerjee_combined_data_1 %>%
#   filter(theta_distribution=="[1, 0, 0, 0]")%>%
#   group_by(network_id, intervention_type,network_size) %>%
#   summarise(
#     utility = mean(utility_to_spread),
#     size = mean(network_size),
#     est = size*0.9,
#   )
# data1 <- data %>%
...
all_summaries_plot_4 <- all_summaries_group_by_id %>%
  ungroup() %>%
  mutate(
  ...
  ggplot(
    aes(
      ...
      ymin = utility_to_spread_lb,
      ymax = utility_to_spread_ub,
  ...
  coord_flip()
all_summaries_plot_4

all_summaries_plot_diff_4 <- all_summaries_group_by_id %>%
  ungroup() %>%
  mutate(
    DELTA = factor(DELTA, levels = sort(levels(DELTA), T)),
  ) %>%
...
all_summaries_plot_2 <- all_summaries_group_by_id %>%
  ungroup() %>%
  mutate(
    DELTA = factor(DELTA, levels = sort(levels(DELTA), T)),
  ) %>%
  ...
  scale_y_log10(breaks = 2^(2:5)) +
  coord_flip()
all_summaries_plot_2

all_summaries_plot_diff_2 <- all_summaries_group_by_id %>%
  ungroup() %>%
  mutate(
    DELTA = factor(DELTA, levels = sort(levels(DELTA), T)),
  ) %>%
  ...
  scale_y_log10(breaks = 2^(2:5)) +
  coord_flip()
all_summaries_plot_diff_2

ggarrange(all_summaries_plot_diff_2, all_summaries_plot_diff_4 + rremove("x.text"),
          labels = c("θ=2","θ=4"),label.x=-0.008,
          label.y = 1,
          ncol = 2, nrow = 1)
